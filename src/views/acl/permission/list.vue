<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="code" label="Prior" />

      <el-table-column prop="toCode" label="Prior value" />
      ...
      <el-table-column label="Operation">
        <template slot-scope="{ row }">
          ...
          <el-button
            ...
            :title="row.level === 4 ? 'Modify the function' : 'Modify the menu'"
          ></el-button>
          <el-button
            :disabled="row.level === 1"
            type="danger"
    ...
    >
      ...
      >
        ...
        <el-form-item label="Prior" prop="code">
...
// 菜单权限校验的规则
const menuRules = {
  name: [{ required: true, message: 'Must have name' }],
  code: [{ required: true, message: 'Prior must type' }],
}

// 按钮功能权限校验的规则
const btnRules = {
  name: [{ required: true, message: 'Must have name' }],
  code: [{ required: true, trigger: 'blur', message: 'Prior value must type' }],
}
  ...
  computed: {
    ...
    */
    dialogTitle() {
      ...
      if (id) {
        return level === 4 ? 'Modify function' : 'Modify menu'
      } else {
        return level === 4
          ? 'Add function'
          : `Add${level === 2 ? 'first' : 'second'}menu`
      }
    },
    ...
    removePermission(permission) {
      this.$confirm('This operation will delete pernamnetly,if continue?', 'hint', {
        ...
        .then(async () => {
          ...
          this.$message.success(result.message || 'Delete successfully!')
          this.fetchPermissionList()
        })
        .catch((error) => {
          if (error === 'cancel') {
      ...
      this.$refs.permission.validate(async (valid) => {
        if (valid) {
          ...
          this.$message.success(
            result.message || `${perm.id ? 'Modify' : 'Add'}Success!`
          )
          this.resetData()
          this.fetchPermissionList()
        }
